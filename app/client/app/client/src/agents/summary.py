...
class SummaryAgent(BaseAgent):
    ...
    def _generate_summary(self, financial_data: Dict[str, Any], sentiment_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        cloud_revenue = financial_data.get("segment_performance", {}).get("cloud_services", {}).get("revenue", "N/A") if isinstance(financial_data.get("segment_performance"), dict) else "N/A"
        cloud_growth = financial_data.get("segment_performance", {}).get("cloud_services", {}).get("growth_rate", 0) if isinstance(financial_data.get("segment_performance"), dict) else 0
        ...
        # Calculate confidence score combining sentiment and financial metrics
        overall_confidence = (sentiment_confidence + min(1.0, operating_margin * 2)) / 2
